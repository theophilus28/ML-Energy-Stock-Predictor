"""
svm preprocessing script

this script provides functions to preprocess energy sector etf data for
support vector machine regression models. the preprocessing transforms raw
csv time series data into 2d tabular format with standardized features.

output format: (samples, features)
- samples: number of training/testing examples (one per trading day)
- features: engineered and standardized features
...
- feature standardization using z-score normalization (required for svm)
- train/test split (2016-2023 for training, 2024 for testing)

important: svm requires feature scaling due to its distance-based nature.
all features are standardized to have mean=0 and std=1 using standardscaler
...
def load_and_preprocess_svm(ticker_path, training_years=5, lag_periods=[1, 5, 10, 20]):
    """
    load csv data and preprocess into standardized 2d format for svm.
    ...
    6. standardizes features using z-score normalization
    7. splits into training and testing sets
    ...
    returns:
        tuple: (x_train, y_train, x_test, y_test, scaler, feature_names)
            x_train (numpy.ndarray): standardized training features, shape (n_train_samples, n_features)
            y_train (numpy.ndarray): training targets (adj close prices)
            x_test (numpy.ndarray): standardized testing features, shape (n_test_samples, n_features)
            y_test (numpy.ndarray): testing targets (adj close prices)
            scaler (StandardScaler): fitted scaler object for potential inverse transformation
            feature_names (list): list of feature column names for interpretation
    
    example:
        x_train, y_train, x_test, y_test, scaler, features = load_and_preprocess_svm(
...
def load_all_tickers_svm(training_years=5, lag_periods=[1, 5, 10, 20]):
    """
    load and preprocess all etf tickers for svm training.
    ...
    returns:
        dict: nested dictionary structure:
              {
                  'oil_and_gas': {
                      'crak': (x_train, y_train, x_test, y_test, scaler, feature_names),
                      'pxe': (x_train, y_train, x_test, y_test, scaler, feature_names),
                      'fcg': (x_train, y_train, x_test, y_test, scaler, feature_names)
                  },
                  'renewable': {...},
                  'nuclear': {...}
              }
    ...
    for subsector, tickers in subsectors.items():
        ...
        for ticker in tickers:
            ...
            all_data[subsector][ticker] = (x_train, y_train, x_test, y_test, scaler, features)
